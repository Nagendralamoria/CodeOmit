  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:title"
          content={`${componentscode?.titlename} - Codeomit`}
        />
        <meta
          property="og:description"
          content={`${componentscode?.codeinst}`}
        />
        ...
        <div className={pagecss.designgrid}>
              ...
              >
                ...
                {componentscode?.JS != "" ? (
                  <>
                    ...
                    <div className={pagecss.maxheightcontainer}>
                      <CopyBlock
                        className={pagecss.maxheightcontainer}
                        style={{ whiteSpace: "pre-line" }}
                        language="js"
          ...
          <div className={pagecss.pagetext}>
            ...
            <div className={pagecss.miniblogdivs}>
              ...
              <p>
                The first step is to create two files: an HTML file and a CSS
                file or if you already have a project file and looking for
                component you can skip these step. The HTML file should be named
                index.html and the CSS file should be named style.css these are
                the first criteria for creating a {componentscode?.name}.{" "}
              </p>

              <h3>Step 2: Insert the Code</h3>
              <p>
                Once you have created the files, insert the provided codes into
                each file and place the flip card code on assumed place. The
                HTML code should go into index.html and the CSS code should go
                into style.css.
              </p>

              <h3>Step 3: Link the CSS File</h3>
              <p>
                In order for the {componentscode?.name} to work properly, you
                must link the CSS file to the HTML file.To do this, insert the
                below code before the title tag.
              </p>
              <CopyBlock
                style={{ whiteSpace: "pre-line" }}
                language="html"
            ...
            <p>
              ...
              the same purpose as JavaScript functions, but work in isolation
              and return HTML. To create a {componentscode?.name} component we
              requires a project file. If you have a project file you can skip
              the first step.
            </p>
            <div className={pagecss.miniblogdivs}>
...
npm start`}
              ...
              <p>
                To do this, create a new file in the src/ components directory
                called {componentscode?.name}.js. In this file, you’ll need to
                import the {componentscode?.name} code at the top and then
                create a class component called {componentscode?.name}. Within
                the component, you’ll need to add the code.
              </p>
              <p>
                And that’s it! You now have a fully functional{" "}
                {componentscode?.name} on your React app. You can customize it
                further by adding additional styling and behavior
              </p>
            </div>
            <h2>
              How To Create {componentscode?.name} Components in AngularJS
            </h2>
            <div className={pagecss.miniblogdivs}>
              ...
              <h3>Step 2: Insert the Code</h3>
              <p>
                When you run this code, a folder with the same name as the
                component will be created. This folder will contain one
                TypeScript file, one HTML file, and one CSS file. Copy the given
                codes and put them in the right files, and put the code for the{" "}
                {componentscode?.name} in the correct place.
              </p>
            </div>
            {componentscode?.codecred != "" ? (
              <>
                <h2>Image Credits</h2>
                <p className={pagecss.blueclr}>{credimg()}</p>
              </>
            ) : null}
          </div>
          <div className={pagecss.maxheightcontainer}></div>
        </div>
